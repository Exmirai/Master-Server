...
import (
	...
	"github.com/exmirai/Master-Server/internal/logging"
	"github.com/exmirai/Master-Server/internal/rand"
...
var server_list = make([]serverInfo, 0, MAX_SERVERS)
var socket *net.UDPConn

var channel_event = make(chan event, 1)
var channel_command = make(chan event, 1)
var channel_udp = make(chan event, 1)
var channel_serverlist = make(chan event, 1)
var channel_sysevent = make(chan os.Signal, 1)
var channel_listrequest = make(chan listRequest, 1)
...
func getInfo(server serverInfo) error {
...
func heartbeat(address *net.UDPAddr, data []string) error {
	...
	info := serverInfo{ip: address, lastHeartbeat: time.Now(), challenge: challenge, status: 1}
...
func processUdpPackets() {
	...
	for {
		...
		switch data[0] {
		case "heartbeat":
			channel_listrequest <- listRequest{t: 0, addr: address, payload: data}
		case "getservers":
			channel_listrequest <- listRequest{t: 2, addr: address, payload: data}
		case "infoResponse":
			channel_listrequest <- listRequest{t: 1, addr: address, payload: data}
		default:
...
func processCmd() {
	...
	for {
		...
		if err != nil {
			if err == io.EOF {
				channel_event <- event{t: EVENT_LOG_WARNING, payload: "Encountered EOF"}
				continue
			}
			channel_event <- event{t: EVENT_ERROR, err: err, sender: channel_command}
		}
		channel_event <- event{t: EVENT_LOG_INFO, payload: fmt.Sprintf("Received: %s\n", text)}
	}
}
...
func StartDeamon(port int) error {
	...
	signal.Notify(channel_sysevent)

	err = initUdpSocket("", port)
	if err != nil {
		logger.Printf("Failed to init udp socket: %s", err)
		return err
	}
	...
	for running == true {
		...
		case signal := <-channel_sysevent:
			logger.Warningf("Got %s signal. Shuttingdown...\n", signal)
			ev := event{t: EVENT_SHUTDOWN}
			channel_serverlist <- ev
			channel_command <- ev
			channel_udp <- ev
			logger.Print("Done")
			running = false
